...
async def forecast(message: Message, command: CommandObject):
   ...
   needed_ids = {
       list(forecast.keys())[i].date():
           (round(sum(list(forecast.values())[i:i + 3]) / 3 - 273.15),
            round(sum(list(forecast.values())[i + 3:i + 6]) / 3 - 273.15),
            round(sum(list(forecast.values())[i + 6:i + 8]) / 2 - 273.15))
   ...
   response = as_list(
       as_marked_section(
           ...
           *[f'{k} \n утро: {v[0]}°C, день: {v[1]}°C, ночь: {v[2]}°C' for k, v in needed_ids.items()],
           marker="🌎",
       ),
   )
   await message.answer(
